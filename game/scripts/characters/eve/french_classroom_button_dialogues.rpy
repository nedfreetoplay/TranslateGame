label eve_classroom_dialogue_eve_intro:
    ...
    show evedesk 4
    eve "О, ты слышал объявление {b}Miss Bissette{/b} о специальном вознаграждении?"
    show evedesk 1
    ...
    player_name "Почему?"
    show evedesk 6
    eve "Как улучшение от B до A даст мого возможностей?"
    ...
    show evedesk 6
    eve "Плюс, {b}Miss Bissette{/b} мальчики нравятся больше."
...
label eve_classroom_dialogue_talent_show_help:
    ...
    show evedesk 5
    player_name "Хмм..."
    player_name "Знаешь, у моего друга {b}Erik{/b} есть {b}караоке-машина{/b} в подвале."
    show evedesk 4
    eve "О, правда?"
    show evedesk 5
...
label eve_classroom_dialogue_adehsive:
    ...
    show evedesk 4
    eve "Ты должен встретиться с {b}Kevin{/b} в {b}научной лаборатории после занятий{/b}."
    eve "Помнишь?"
    show evedesk 5
    player_name "Ох, да точно. Спасибо, {b}Eve{/b}!"
    return

label eve_classroom_dialogue_bissettes_reward:
    show evedesk 5
    player_name "Ты собираешься записаться на репетиторство к {b}Miss Bissette{/b}?"
    show evedesk 4
    eve "Я уже делаю довольно хорошо, так что даже не стоит пытаться."
    show evedesk 2
    player_name "Почему нет?"
    show evedesk 6
    eve "Как переход от B к A считается значительным улучшением?"
    eve "У тебя больше шансов выиграть..."
    show evedesk 5
    player_name "Я?"
    show evedesk 1
    ...
    eve " У тебя есть много возможностей для улучшения."
    show evedesk 6
    eve "Плюс, {b}Miss Bissette{/b} благоволит парням."
    eve "Тебе стоит серьезно подумать об этом."
    return
