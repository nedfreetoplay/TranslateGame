label button_ross_need_linens:
    scene location_school_art_closeup
    show player 10f zorder 1 at right
    show mia 2f zorder 0 at Position(xpos=0.55, ypos=1.0)
    with dissolve
    player_name "Я думал что мы должны были рисовать сегодня на конкурс?"
    show player 11f
    show mia 6f
    mia "Вот что {b}Miss Ross{/b} сказала."
    show player 10f
    show mia 2f
    player_name "Хмм, ну,где же она?"
    show player 11f
    show mia 6f
    mia "Я не знаю."
    show mia 2f
    player_name "..."
    show mia 6f
    mia "Эй, {b}[firstname]{/b}..."
    show player 10f
    show mia 2f
    player_name "Да?"
    show player 11f
    show mia 6bf
    mia "Я хотела извиниться за вчерашнее."
    show player 10f
    show mia 2f
    player_name "мм?"
    show player 11f
    show mia 6f
    mia "Ты знаешь, все обнаженные вещи..."
    show mia 6bf
    mia "... а так же гудение."
    show mia 2f
    player_name "..."
    show mia 6f
    mia "Я действительно не знаю что на меня нашло!"
    show mia 2f
    show player 2f
    player_name "Все нормально, {b}Mia{/b}.Я хорошо провел время."
    show mia 6f
    show player 1f
    mia "Правда?"
    show player 2f
    show mia 1f
    player_name "Да!"
    player_name "Я думал, что тебе тоже было весело?"
    show player 1f
    show mia 4f
    mia "Хех,да. Это было весело..."
    show mia 6bf
    mia "... Это просто..."
    show mia 6f
    mia "... Это было не очень женственно."
    mia "Так что, я извиняюсь, {b}[firstname]{/b}."
    show mia 2f
    show player 2f
    player_name "{b}Mia{/b}, правда, все в порядке."
    player_name "Мне нравится эта твоя сторона."
    show mia 4f
    show player 1f
    mia "Это так?"
    show player 2f
    show mia 1f
    player_name "Да, мы все просто весело проводили время!"
    player_name "В этом нет ничего плохого."
    show player 1f
    show mia 3f
    mia "... Да, я думаю ты прав."
    show mia 4f
    mia "Спасибо, {b}[firstname]{/b}!"
    show mia 1f
    show player 2f
    player_name "О, вот она идет!"
    show mia 4 at Position(xpos=0.65, ypos=1.0)
    show ross 24 at left
    with dissolve
    show player 1f
    mia "Здравствуйте, {b}Miss Ross{/b}!"
    mia "Все в порядке?"
    show mia 1
    show ross 25
    ross "Привет, Сладенькая."
    show ross 25b
    ross "Агх, Я только что была в кабинете {b}Директрисы Smith's{/b}..."
    show ross 25
    ross "Она все говорила и говорила о портрете, который мы ей обещали.."
    show ross 24
    show player 10f
    player_name "Вы имеете в виду что нам действительно нужно это сделать?"
    show player 11f
    show ross 25b
    ross "Боюсь что да."
    show ross 25
    ross "У нее тоже есть некоторые специфические требования..."
    show ross 24
    show mia 6
    mia "... Но что насчет конкурса?"
    mia "{b}[firstname]{/b} все еще собирается что-то нарисовать для этого, верно?"
    show mia 2
    show ross 23
    ross "Агх, я знаю... у меня все было тоже спланировано!"
    show ross 25b
    ross "Идеальная тем."
    show ross 25
    ross "Идеальная модель."
    show mia 5
    ross "Это был бы беспроигрышный победитель!"
    show ross 23
    ross "Но {b}Директриса Smith{/b} пришла и бросила гаячный ключ во все это..."
    show mia 1
    show ross 22
    show player 10f
    player_name "Так что мы будем делать, у нас нет времени на две картины!"
    show player 11f
    show ross 25
    ross "Мы просто придется принять удары на себя и надеятся что картина {b}Директриса Smith's{/b}  будет достаточно хороша, чтобы выиграть конкурс..."
    show player 10f
    show ross 24
    player_name "Мы собираемся представить картину {b}Директрисы Smith{/b} на конкурс?!"
    show player 11f
    show mia 2
    show ross 25b
    ross "Просто сделай все возможное, {b}[firstname]{/b}."
    show mia 6
    show ross 24
    mia "По крайней мере на это должно быть интересно смотреть."
    ross "..."
    show mia 2
    show ross 25
    ross "К сожалению, золотце.Ты и я будем сидеть здесь пока он будет там."
    ross "Только {b}[firstname]{/b} разрешенно находиться в комнате."
    show ross 25b
    ross "Это еще одно из условий {b}Директрисы Smith's{/b}."
    show player 10f
    show ross 24
    player_name "Серьезно!?"
    show ross 25
    show player 11f
    ross "Поверь мне, Я тоже не в восторге от этого..."
    show ross 23
    ross "Черт побери!"
    ross "Я только начинала чувствовать себя уверенно на своей работе, и это должно было случиться."
    show ross 22
    show mia 3
    mia "Не волнуйтесь, {b}Miss Ross{/b}. Я уверена что{b}[firstname]{/b} найдет выход чтобы у нас все получилось!"
    show mia 5
    player_name "..."
    show ross 25
    ross "Я надеюсь ты права."
    show mia 1
    show ross 25b
    ross "Нам лучше всего начать с подготовки.."
    show ross 24
    show player 10f
    player_name "Подготовки?!"
    show ross 25
    show player 11f
    ross "О, Да, у нас есть куча работы,прежде чем ты сможешь начать рисовать."
    show ross 25b
    ross "Наши полки с запасами абсолютно пустые!"
    show ross 25
    ross "Нам нужно найти краску и холст."
    show player 10f
    show ross 24
    player_name "Где бы мы могли найти все это?"
    mia "..."
    show ross 25
    ross "Ну, Я бы могла,нам сделать холст из достаточного количества белого {b}постельного белья{/b}."
    show ross 24
    show player 10f
    player_name "Взаправду?"
    show player 11f
    show mia 4
    mia "Я могу это достать."
    show player 10f
    show mia 1
    player_name "Ммм?"
    show ross 25
    show player 11f
    ross "В самом деле?"
    show mia 3
    mia "Да, не проблема.В моей церкви тонна белого {b}постельного белья{/b}."
    show mia 5
    show player 10f
    player_name "... И ты думаешь что они просто позволят взять нам немного?"
    show mia 3
    show player 11f
    mia "Да,я думаю да!"
    mia "В конце концов, давать нуждающимся-довольно большой принцип католицизма."
    show mia 5
    show player 10f
    player_name "Ммм, чтож это удобно."
    show mia 4
    show player 11f
    mia "Я сейчас же этим займусь."
    show mia 1
    show ross 25
    ross "Нет, погоди-ка."
    ross "{b}[firstname]{/b} может заняться {b}постельным бельем{/b}."
    show ross 24
    show player 10f
    player_name "... Я могу?"
    show ross 25
    show player 11f
    ross "Точно, просто скажите церковным людям что {b}Mia{/b} отправила тебя."
    show ross 28b with dissolve
    ross "{b}Mia{/b}, Я хочу, чтобы ты отнесла это в {b}Cosmic Cumics{/b} в торговом центре и сделала заказ."
    show ross 24
    show player 10f
    show mia 68 at Position(xpos=0.6, ypos=1.0) with dissolve
    player_name "Что это?"
    show ross 25b
    show player 11f
    ross "{b}Дизайн костюма Директрисы Smith's{/b}."
    show player 10f
    show ross 24
    player_name "{b}Костюма{/b}?!"
    show player 11f
    show mia 66
    pause
    show mia 67
    mia "... О, боже..."
    show mia 68
    player_name "..."
    show mia 67
    mia "Ясно,Мэм.Я этим займусь!"
    show player 10f
    hide mia with dissolve
    player_name "Подожди,что за {b}костюм{/b}?!"
    show ross 25b
    show player 11f
    ross "Не волнуйся, ты скоро об этом узнаешь."
    show ross 25
    ross "Теперь поторопись и принеси мне эти {b}Постельное белье{/b} так мы сможем сосредоточиться на проблеме с краской!"
    show ross 24
    show player 10f
    player_name "Агх, здорово."
    return

label button_ross_get_linens:
    scene location_school_art_closeup
    show ross 11 at left
    show player 1f at right
    with dissolve
    ross "Ты принес {b}Белье{/b}?"
    show ross 10
    show player 2f
    player_name "Нет,Я их еще не получил."
    show ross 11
    show player 1f
    ross "У нас мало времени, {b}[firstname]{/b}!"
    ross "Мы должны поторопиться и забрать их у {b}Церкви Mia{/b} если ты хочешь представить свою картину во время, на конкурсе!"
    show ross 10
    show player 2f
    player_name "Не волнуйтесь, Я этим займусь!"
    return

label button_ross_has_linens:
    scene location_school_art_closeup
    show player 592f at right
    show ross 11 at left
    ross "Это то самое {b}постельное белье{/b}?"
    show player 593f
    show ross 10
    player_name "Ага, они немного потрепаны.Я надеюсь,все в порядке?"

    show player 592f
    show ross 11
    ross "О, это не будет проблемой.Там много полезного материала."
    show player 2f with dissolve
    show ross 10
    player_name "Хвала небесам."
    show player 10f
    player_name "Так что теперь?"
    show ross 25
    show player 11f
    ross "{b}Нам нужна краска{/b}, помнишь?"
    show ross 25b
    ross "И она нам нужна бесплатно."
    show ross 25
    ross "У меня нет бюджета, чтобы купить любую..."
    show player 10f
    show ross 24
    player_name "Это не оставляет нам много вариантов."
    show ross 10b with dissolve
    show player 11f
    ross "Хмм..."
    player_name "..."
    show ross 11 with dissolve
    ross "{b}Иди поговори с Евой{/b} посмотрим есть ли у неё какя-нибудь и может ли она нам одолжить."
    ross "Нам действительно не нужно много для одной картины."
    ross "Я должна начать работать над этим холстом."
    show player 10f
    show ross 10
    player_name "Хорошо."
    return

label button_ross_get_paint_grace:
    scene location_school_art_closeup
    show ross 25 zorder 1 at left
    show mia 8b zorder 0 at Position(xpos=0.45, ypos=1.0)
    with dissolve
    ross "... Чтож, я не знаю, что мы будем делать!"
    ross "Она действительно уволит меня!"
    show ross 24
    pause
    show player 589f at right with dissolve
    show ross 10
    pause
    show ross 11
    ross "{b}*Задыхаясь,вздох*{/b} Вот он!"
    show ross 10
    show mia 10f at Position(xpos=0.35, ypos=1.0) with dissolve
    mia "О, {b}[firstname]{/b}! Слава богу!!"
    show mia 7f
    show ross 11
    ross "Мы так волновались, что ты не успеешь вернуться вовремя!"
    show player 590f
    show ross 10
    player_name "Я здесь.Что происходит?"
    show player 589f
    show mia 12bf
    mia "{b}Annie{/b} просто приходила,чтобы забрать костюмы. {b}Директриса Smith{/b} и она переодевается в него прямо сейчас!"
    show player 590bf
    show mia 8bf
    player_name "Она хочет, чтобы я нарисовал ее прямо сейчас?!"
    player_name "... И с каких пор {b}Annie{/b} участвует?!"
    show player 589bf
    show ross 25
    ross "{b}Директриса Smith{/b} хочет, чтобы она тоже была на картине."
    show ross 24
    show mia 12f
    mia "У них такие странные отношения..."
    show mia 8f
    show player 590bf
    player_name "Готов ли я к этому?"
    show player 589bf
    show ross 25
    ross "Нет времени беспокоиться об этом сейчас!"
    ross "Это краска?"
    show ross 24
    show player 590f
    player_name "Да, это от {b}Eve{/b}'s сестры, {b}Grace{/b}."
    show ross 25
    show player 589f
    ross "Только основные цвета?"
    show ross 24
    show player 590f
    player_name "Да, остальные нам придется смешивать."
    show player 589f
    show ross 25
    ross "Блин!"
    ross "Мы должны смешать их быстро!"
    ross "Дайте мне взять политру!"
    hide ross
    with dissolve
    show mia 9f

    mia "Это так волннительно!"
    show mia 11f
    player_name "..."
    show mia 12f
    mia "Ты нервничаешь?"
    show mia 8f
    show player 590bf
    player_name "Немного."
    player_name "Я не хочу чтобы {b}Мисс Ross{/b} потеряла свою работу!"
    show player 589bf
    show mia 9f
    mia "Не волнуйся, {b}[firstname]{/b}.У тебя все получится!"
    show player 590f
    show mia 11f
    player_name "Спасибо, {b}Mia{/b}."
    player_name "Я так рад, что ты согласилась быть моим партнером во всем этом!"
    show player 589f
    show mia 10f
    mia "... Я тоже!"
    show mia 7f
    show ross 2b zorder 1 at left
    ross "Отлично, давайте быстрее их смешаем, прежде чем они вернутся!"
    return

label button_ross_waiting_for_contest:
    scene location_school_art_closeup
    show ross 10 at left
    show player 2f at right
    with dissolve
    player_name "Что-нибудь слышно о конкурсе?"
    show player 1f
    show ross 11
    ross "Пока ничего."
    ross "Я дам тебе знать,как только что-нибудь узнаю.."
    show player 2f
    show ross 10
    player_name "Хорошо."
    return

label button_ross_contest:
    scene location_school_art_closeup
    show ross 11 at left
    show iwanka 3 zorder 1 at Position(xpos=0.45, ypos=1.0)
    show iwankae 1 zorder 2 at Position(xpos=0.39, ypos=0.965)
    with dissolve
    ross "О, да! Он чрезвычайно талантлив, не так ли?"
    show ross 10
    show player 11f zorder 0 at right with dissolve
    pause
    show player 10f
    player_name "{b}Мисс Ross{/b}?"
    show player 11f
    show ross 11
    ross "О! А вот и он!"
    show iwanka 3f at Position(xpos=0.45, ypos=1.0)
    show iwankae 1f at Position(xpos=0.51, ypos=0.965)
    with dissolve
    pause
    show iwanka 2f
    show ross 11
    ross "Подойди сюда, {b}[firstname]{/b}.Кое-кто хочет с тобой познакомиться."
    show ross 10
    player_name "..."
    show ross 11
    ross "Это {b}Дочь мэра{/b}."
    show ross 10
    show player 10f
    player_name "Вау, Серьезно?"
    show player 11f
    show iwanka 4f
    iwanka "Приятно познакомиться, {b}[firstname]{/b}!"
    iwanka "Меня зовут {b}Iwanka{/b}."
    show iwanka 2f
    show player 10f
    player_name "П-привет."
    show player 11f
    show ross 11
    ross "Ой, прости его. Он немного стесняется рядом с красивой женщиной!"
    show ross 10
    show iwanka 4f
    iwanka "Все в порядке!"
    hide iwankae
    show iwanka 5f at Position(xpos=0.58, ypos=1.0)
    with dissolve
    iwanka "Мой отец хотел, чтобы я лично передала это вам вместе с его поздравлениями.."
    show player 582f
    show iwanka 2f at Position(xpos=0.45, ypos=1.0)
    with dissolve
    player_name "..."
    show player 584f
    player_name "... Это?"
    show player 583f
    show iwanka 4f
    iwanka "Это твои призовые деньги!"
    show player 584f
    show iwanka 2f
    player_name "Подожди, в смысле,моя картина выиграла?!"
    show player 583f
    show iwanka 4f
    iwanka "Все верно!Первое место!"
    show player 584f
    show iwanka 2f
    player_name "Awesome!"
    show player 583f
    show iwanka 4f
    iwanka "Моему отцу очень понравилась твоя работа!"
    iwanka "На самом деле, он сделал копию для своей частной коллекции."
    iwanka "Он даже подумывает нанять тебя, чтобы ты нарисовал всю семью Рампов.."
    show player 584f
    show iwanka 2f
    player_name "Всерьез?"
    show player 583f
    iwanka "Мммхмм..."
    show iwanka 4f
    iwanka "Я взяла твой номер телефона от {b}Мисс Ross{/b}. Я надеюсь ты не возражаешь?"
    show player 584f
    show iwanka 2f
    player_name "Д-да, безусловно!"
    show player 583f
    show iwanka 4f
    iwanka "Очень хорошо, мы будем на связи.."
    iwanka "Поздравляю еще раз и хорошего дня!"
    show player 584f
    show iwanka 3f
    player_name "Да, тебе тоже!"
    hide iwanka
    with dissolve
    player_name "Я не могу в это поверить, мы выйграли, {b}Мисс Ross{/b}!"
    show player 583f
    show ross 27
    ross "Ты выйграл, {b}[firstname]{/b}!"
    ross "... И я очень тобой горжусь!"
    show player 584f
    show ross 26
    player_name "Вот, эти деньги для вас."
    show player 1f
    show ross 33
    with dissolve
    ross "О боже мой!"
    show ross 32
    pause
    show ross 31
    ross "... Это просто..."
    show ross 32
    pause
    hide player
    show ross 21
    with dissolve
    ross "Ты мой герой, {b}[firstname]{/b}!"
    show ross 20
    pause
    show ross 21
    ross "Ты знаешь..."
    show ross 20
    pause
    show ross 21
    ross "... Есть еще одна художественная техника, которой я хотела бы тебя научить."
    show ross 13
    show player 10f at right
    with dissolve
    player_name "Есть?"
    show player 11f
    show ross 12
    ross "Мммхмм..."
    show ross 13
    ross "... Почему бы тебе не прийти ко мне в кабинет после школы, и я покажу тебе."
    show ross 12
    show player 2f
    player_name "Хорошо, я пойду скажу {b}Mia{/b}!"
    show ross 25
    show player 1f
    ross "Нет! Подожди."
    show ross 24
    show player 11f
    player_name "..."
    show ross 25
    ross "Я боюсь,что {b}Mia{/b} еще не готова к этой конкретной технике."
    show ross 24
    show player 10f
    player_name "О, она слишком продвинутая для нее?"
    show ross 13
    show player 11f
    ross "Хех, Да. Что-то вроде того."
    ross "Она однажды её освоит, не волнуйся..."
    show ross 12
    show player 10f
    player_name "Х-хорошо,Я думаю, увидимся после школы?"
    show ross 13
    show player 11f
    ross "Я буду ждать."
    return
# Decompiled by unrpyc: https://github.com/CensoredUsername/unrpyc
