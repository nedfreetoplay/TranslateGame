...
label rapbattle_listing:
    ...
    $ chico1 = Rap(line = "Чико, почему ты такая стерва? \nТвоей разбитой губе понадобится ____.", rhymes = ["{color=#FFD700}швы{/color}", "заплатки", "поцелуй"], answer = "{color=#FFD700}швы{/color}")
    $ chico2 = Rap(line = "Ты уверен, что тебя зовут не Рик?\nПотому что сейчас ты ведешь себя ____.", rhymes = ["{color=#FFD700}хуй{/color}", "писюн", "задница"], answer = "{color=#FFD700}хуй{/color}")
    $ chico_list = [chico1, chico2]
    $ chico3 = Rap(line = "Ты хочешь команду? Я дам тебе три.\nУ тебя не будет других богов \nпередо ____.", rhymes = ["{color=#FFD700}мной{/color}", "Као", "Иисусом"], answer = "{color=#FFD700}мной{/color}")
    $ chico4 = Rap(line = "Преклонитесь передо мной и уважайте мой ход.\nя даю команды сейчас,\nэто моё ____.", rhymes = ["{color=#FFD700}шоу{/color}", "рэп", "жизнь"], answer = "{color=#FFD700}шоу{/color}")
    $ chico_list2 = [chico3, chico4]
    $ chico5 = Rap(line = "Кроме Евы, все что я вижу здесь, только позера. \nТы сделал ход, \nно я буду ____.", rhymes = ["{color=#FFD700}близко{/color}", "лучше", "Иисус"], answer = "{color=#FFD700}близко{/color}")
    $ chico6 = Rap(line = "И правда мужик? Один из твоих родственников?\nЯ мог бы вытянуть лучшие рэпы в \n мусорном ____.", rhymes = ["сумка", "грузовик", "{color=#FFD700}ведре{/color}"], answer = "{color=#FFD700}ведре{/color}")
    $ chico_list3 = [chico5, chico6]
    $ chico7 = Rap(line = "Эй йоу, Чико, ты действительно хочешь рэпа? \nты читаешь так, будто у тебя перья на ____.", rhymes = ["{color=#FFD700}твоей кепке{/color}", "твоих волосах", "твоей заднице"], answer = "твоей кепке{/color}")
    $ chico8 = Rap(line = "Йоу, твои рифмы такие ненормальные; \nВсе что тебе нужно это ____.", rhymes = ["новый сникерс", "{color=#FFD700}закадровый смех{/color}", "мамкины спагетти"], answer = "{color=#FFD700}закадровый смех{/color}")
    $ chico_list4 = [chico7, chico8]

    $ chad_time = 6.0
    $ chad1 = Rap(line = "Очень приятно познакомиться с тобой Чад и я \nрад что я сделал, потому что твое чтение было \nслабым, а ты сложен как ____.", rhymes = ["{color=#FFD700}ребенок{/color}", "женщина", "слон"], answer = "{color=#FFD700}ребенок{/color}")
    $ chad2 = Rap(line = "Я могу быть в твоем классе,но ты будешь \nучиться у меня. Я починю твое мастерство,\nи я это даже сделаю  ____.", rhymes = ["{color=#FFD700}бесплатно{/color}", "массаж", "все"], answer = "{color=#FFD700}бесплатно{/color}")
    $ chad3 = Rap(line = "Ты ничему не можешь меня научить,ты не освоил ни хрена. \nТы можешь получить\nэто место сейчас,я думаю пришло время тебе ____.", rhymes = ["{color=#FFD700}присесть{/color}", "убираться", "отъебаться"], answer = "{color=#FFD700}присесть{/color}")
    $ chad4 = Rap(line = "Твоя проза слаба,и твой \nсклад ума не стоит упомиминания, Твоё \nчтение неудачника, и это принесет тебе ____.", rhymes = ["{color=#FFD700}арест{/color}", "аутизм", "овации"], answer = "{color=#FFD700}арест{/color}")
    $ chad_list = [chad1, chad2, chad3, chad4]
    $ chad5 = Rap(line = "Этот снова уставший рэп? Хорошо Чад, \nдавай сделаем это диким, тебе нравится эта школа \nтварь, мм? Чтож, давай тогда, ____.", rhymes = ["{color=#FFD700}ребенок{/color}", "чувак", "лузер"], answer = "{color=#FFD700}ребенок{/color}")
    $ chad6 = Rap(line = "Я здесь не затем, чтобы впечатлять тебя, но поверь,\nя оставлю впечатление,ты не будешь\nв стостоянии ходить после того,как я закончу с этим ____.", rhymes = ["{color=#FFD700}сеансом{/color}", "дерьмом", "линией"], answer = "{color=#FFD700}сеансом{/color}")
    $ chad7 = Rap(line = "Ты симпатичный парень, так что да, давай поговорим о тебе, \nи о том, как я уничтожу тебя и эту пидорскую____.", rhymes = ["{color=#FFD700}прическу{/color}", "стиль", "рожу"], answer = "{color=#FFD700}прическу{/color}")
    $ chad8 = Rap(line = "Ты хочешь плюнуть на естественную науку о жаре\n конвекции? Я зажарил тебя \n, и это дало твоим мальчикам ____.", rhymes = ["{color=#FFD700}эрекцию{/color}", "аутизм", "мурашки по коже"], answer = "{color=#FFD700}эрекцию{/color}")
    $ chad_list2 = [chad5, chad6, chad7, chad8]
    $ chad9 = Rap(line = "Давай не будем говорить о твоей такой тонкой коже\n я все в твоем районе, \nи ты знаешь, что это мой ____.", rhymes = ["{color=#FFD700}биз{/color}", "женшина", "слон"], answer = "{color=#FFD700}биз{/color}")
    $ chad10 = Rap(line = "Да, я ношу сандалии, потому что кроссовки тают, \nмой огонь горячее твоего,когда я снимаю свой ____.", rhymes = ["{color=#FFD700}ремень{/color}", "штаны", "парик"], answer = "{color=#FFD700}ремень{/color}")
    $ chad11 = Rap(line = "Мне насрать на твое остроумие, у меня острый\nкак клинок, когда я покончу с твоей задницей, \nя зайду в торговый центр и ____.", rhymes = ["{color=#FFD700}перепихнусь{/color}", "напьюсь", "выброшусь"], answer = "{color=#FFD700}перепихнусь{/color}")
    $ chad12 = Rap(line = "Так что это было весело в твоем классе, жаль что \n твой рэп был плохим, я выпустился с этой \n школы, это конец, До свидания ____.", rhymes = ["{color=#FFD700}Чад{/color}", "все", "жены"], answer = "{color=#FFD700}Чад{/color}")
    $ chad_list3 = [chad9, chad10, chad11, chad12]

    $ tyrone_time = 6.0
    $ tyrone1 = Rap(line = "Не говори о моей семье, я тебя нахуй порежу. \nУ меня есть в доме меч, острый, как ____.", rhymes = ["{color=#FFD700}гинсу{/color}", "джутсу", "Винду"], answer = "{color=#FFD700}гинсу{/color}")
    $ tyrone2 = Rap(line = "Но ничто так не остро, как ритм в этом рэпе, \nкак нож в твоем сердце, просто нельзя ____.", rhymes = ["{color=#FFD700}устранить{/color}", "следовать", "опровергнуть"], answer = "{color=#FFD700}устранить{/color}")
    $ tyrone3 = Rap(line = "Ты зажен во мне кровожадное намерение, \nМне нужно успокоиться, прежде чем я ____.", rhymes = ["{color=#FFD700}истощусь{/color}", "согнусь", "запрусь"], answer = "{color=#FFD700}истощусь{/color}")
    $ tyrone4 = Rap(line = "Дай мне немного кексов, парень, \nмне тоже нужно облегчение, так что не возвышайся ____.", rhymes = ["{color=#FFD700}высоко{/color}", "сторону", "шагай"], answer = "{color=#FFD700}высоко{/color}")
    $ tyrone5 = Rap(line = "Ты единственный раста, которого я когда-либо встречал, \nтоже самый гейский, если бы мне пришлось ____.", rhymes = ["{color=#FFD700}ставить{/color}", "переживать", "потеть"], answer = "{color=#FFD700}ставить{/color}")
    $ tyrone6 = Rap(line = "Ты не на улице, ты,отмечен как заноза в заднице, \nя был повсюду вокруг, ты не оставил этот ____.", rhymes = ["{color=#FFD700}парк{/color}", "темноту", "искру"], answer = "{color=#FFD700}парк{/color}")
    $ tyrone7 = Rap(line = "И да, я помню,как я оказался, я шлепнул двух сук, ты вскоре будешь ____.", rhymes = ["{color=#FFD700}третим{/color}", "свободным", "собой"], answer = "{color=#FFD700}третим{/color}")
    $ tyrone8 = Rap(line = "Мне не нужно заканчивать, я обезвредил твою бомбу, \nзаменил ее своим собственным сжиганием ____.", rhymes = ["{color=#FFD700}напалма{/color}", "пальмы", "псалом"], answer = "{color=#FFD700}напалма{/color}")
    $ tyrone_list = [tyrone1, tyrone2, tyrone3, tyrone4, tyrone5, tyrone6, tyrone7, tyrone8]
    $ tyrone9 = Rap(line = "Я говорил тебе один раз раньше, и я скажу тебе еще раз, \nоставь мою семью в покое, \nпрежде чем я передам гнев в прошлое ____.", rhymes = ["{color=#FFD700}десяти{/color}", "шариковой ручкой", "лощина"], answer = "{color=#FFD700}десяти{/color}")
    $ tyrone10 = Rap(line = "Я включу обогреватель до одиннадцати, когда я доберусь до тебя,\nя мистер T. в этом матче, и мне жаль тебя, ____.", rhymes = ["{color=#FFD700}фуу{/color}", "r2d2", "тоже"], answer = "{color=#FFD700}фуу{/color}")
    $ tyrone11 = Rap(line = "Ты представляешь себя драконом? \nСучка, это очень мило. \nУ тебя вес ящерицы в дешевой заднице ____.", rhymes = ["{color=#FFD700}костюма{/color}", "дороги", "добычи"], answer = "{color=#FFD700}костюма{/color}")
    $ tyrone12 = Rap(line = "Я настоящий истребитель драконов, \nи,истребительница кисок,в придачу \nу меня в логове гарем,а что есть у тебя? ____?", rhymes = ["{color=#FFD700}Фрукт{/color}", "привлекательность", "безмолвность"], answer = "{color=#FFD700}Фрукт{/color}")
    $ tyrone13 = Rap(line = "Ты не смог бы поджарить меня, \nдаже если бы у тебя был мой огонь, ты проиграешь эту битву теперь, \nкогда спровоцировал мой ____.", rhymes = ["{color=#FFD700}гнев{/color}", "дробилку", "оруженосца"], answer = "{color=#FFD700}гнев{/color}")
    $ tyrone14 = Rap(line = "Ты думаешь, что избил меня, когда я истекал кровью? \nЭто история неудачника, в этом вся моя мотивация  ____.", rhymes = ["{color=#FFD700}необходима{/color}", "бусина", "подвиг"], answer = "{color=#FFD700}необходима{/color}")
    $ tyrone15 = Rap(line = "Я получил удар, как пуля в мозг, ты передо мной? \nТы должен быть ____.", rhymes = ["{color=#FFD700}безумный{/color}", "крейн", "глупый"], answer = "{color=#FFD700}безумный{/color}")
    $ tyrone16 = Rap(line = "Я видела тебя насквозь и знаю, кто ты такой. \nИди к себе домой мальчик, \nвозьми свою растовскую  ____ задницу.", rhymes = ["{color=#FFD700}далекую{/color}", "бар", "машина"], answer = "{color=#FFD700}далекую{/color}")
    $ tyrone_list2 = [tyrone9, tyrone10, tyrone11, tyrone12, tyrone13, tyrone14, tyrone15, tyrone16]
    $ tyrone17 = Rap(line = "Подожди, дай мне начать битву на этот раз. \nЭто упреждающий удар, надрать тебе задницу ____.", rhymes = ["{color=#FFD700}стишком{/color}", "грязью", "центами"], answer = "{color=#FFD700}стишком{/color}")
    $ tyrone18 = Rap(line = "Я бил твоих ребят, ты последний в списке, \nЧико командовал, а Чад был ____.", rhymes = ["{color=#FFD700}уволен{/color}", "шипел", "бухой"], answer = "{color=#FFD700}уволен{/color}")
    $ tyrone19 = Rap(line = "Я бы сказал, что уже убил тебя, \nно ты все еще дышишь, и это скоро будет исправлено, \nты, сырая ____ задница.", rhymes = ["{color=#FFD700}языческая{/color}", "фримэн", "спермой"], answer = "{color=#FFD700}языческая{/color}")
    $ tyrone20 = Rap(line = "Я слышал твой рэп и слушал, как ты сорвался, \nпришло время уложить вас в постель с физраствором ____ физраствором.", rhymes = ["{color=#FFD700}капельничным{/color}", "клип", "вырвано"], answer = "{color=#FFD700}капельничным{/color}")
    $ tyrone21 = Rap(line = "Я бы посмотрел тебе в глаза, \nно я не вижу сквозь дым, ты думал, \nчто плюешься огнем, но твоя задница ____.", rhymes = ["{color=#FFD700}посмешище{/color}", "чувак", "дубак"], answer = "{color=#FFD700}посмешище{/color}")
    $ tyrone22 = Rap(line = "У меня лед в венах, я порочный и холодный. \nЯ прикончу тебя прямо здесь, \nу тебя не будет шанса получить ____.", rhymes = ["{color=#FFD700}старье{/color}", "смелость", "холод"], answer = "{color=#FFD700}старье{/color}")
    $ tyrone23 = Rap(line = "Запомни мои слова, когда я положил твою задницу на стул, \nты просил об этом в начале, \nкогда ты назвал меня ____.", rhymes = ["{color=#FFD700}клоуном{/color}", "педиком", "хмурым"], answer = "{color=#FFD700}клоуном{/color}")
    $ tyrone24 = Rap(line = "Кто смеется сейчас, сучка? Ты не очень хорошо выглядишь, \nя здесь, я показал тебе, это мой ____.", rhymes = ["{color=#FFD700}район{/color}", "дерево", "состояние"], answer = "{color=#FFD700}район{/color}")
    $ tyrone_list3 = [tyrone17, tyrone18, tyrone19, tyrone20, tyrone21, tyrone22, tyrone23, tyrone24]
    $ tyrone25 = Rap(line = "Я остановлю тебя прямо здесь, ясно, что ты закончил. \nУ тебя закончился рэп, и я должен ____.", rhymes = ["{color=#FFD700}тонну{/color}", "телефон", "ничего"], answer = "{color=#FFD700}тонну{/color}")
    $ tyrone26 = Rap(line = "Так что я возьму твою лодку и поплыву по рэп-морю. \nНепобедимый и бесспорный, лучший ____.", rhymes = ["{color=#FFD700}Мс{/color}", "Дс", "ПК"], answer = "{color=#FFD700}Мс{/color}")
    $ tyrone27 = Rap(line = "Я победил твою команду и теперь ты кланяешься, \nпосмотри на меня, сука, я ____ капитан.", rhymes = ["{color=#FFD700}сейчас{/color}", "корова", "голубок"], answer = "{color=#FFD700}сейчас{/color}")
